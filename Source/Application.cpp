...
int main() {
	...
	if (graphene::editor::frameworkInit(graphene::editor::frameworkConfig())) {
		while (graphene::editor::newFrame()) {
			...
			//if (graphene::editor::graphic::swapReady) {
				...
				for (auto v : graphene::editor::graphic::frontBuffer->vertices) {
					ImGui::GetWindowDrawList()->AddCircleFilled(
						ImVec2(ImGui::GetCursorScreenPos().x + v.position.x * 200.0f - 100.0f, ImGui::GetCursorScreenPos().y + v.position.y * 200.0f - 100.0f), 5.0f,
						ImGui::ColorConvertFloat4ToU32(ImVec4(1.0f, 1.0f, 1.0f, 1.0f)), 20);
				}
