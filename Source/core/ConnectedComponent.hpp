...
class ConnectedComponent {
  ...
  public:
	...
	bool isValidComponent() const { return validComponent; }

	Vertex* getRootVertex() { return root; }

	void setRootVertex(Vertex* v) { root = v; }
	...
	explicit ConnectedComponent(Vertex* v) {
		root = v;
		ImGui::ColorConvertHSVtoRGB(graphene::tools::random::genRandomNorm() * 360.0f, 0.9f, 0.8f, color.x, color.y,
									color.z);
		uid = graphene::tools::random::generate64();
	}

	void updateConnectedComponent(std::unordered_map<Vertex*, std::unordered_set<Vertex*>>& graph,
								  std::unordered_map<Vertex*, bool>& visited, Vertex* v) {
		v->component = this;
