...
class Graph {
  public:
	...
	void updateConnectedComponent() {
		graphene::tools::profiler::Timer timer;
	...
	std::unordered_set<ConnectedComponent*>::iterator deleteConnectedComponent(ConnectedComponent* component,
																			   bool deleteContent) {
	...
	unsigned int getVertexCount() const { return graph.size(); }
	...
	void deleteVertex(Vertex* v) {
		...
		if (v->component->getRootVertex() == v) {
			if (v->component->adjList.size() == 1) { // v is the only vertex in the component: delete the component (it
													 // should not have adjacent edges too)
				...
				if (v == v->component->getRootVertex()) {
					...
					LOG_VERBOSE("transfer component " + v->component->uid + " root to " +
								v->component->getRootVertex()->uid);
				}
			}
		}
		graph.erase(v);
