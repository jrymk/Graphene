    ...
    namespace LogWindow {
        ...
        void show() {
            ...
            if (ImGui::Begin("Log", &open, ImGuiWindowFlags_MenuBar)) {
                if (ImGui::BeginMenuBar()) {
                    if (ImGui::BeginMenu("Options")) {
                        if (ImGui::MenuItem("Clear all"))
                            graphene::tools::log::logBuffer.clear();
                    ...
                    if (ImGui::BeginMenu("Filter")) {
                        ...
                        int tempFilter = -1;
                        if (fVerbose && filter != 0) tempFilter = 0;
                        if (fDebug && filter != 1) tempFilter = 1;
                        if (fInfo && filter != 2) tempFilter = 2;
                        if (fWarning && filter != 3) tempFilter = 3;
                        if (fError && filter != 4) tempFilter = 4;
                        if (tempFilter != -1)
                            filter = tempFilter;
                ...
                for (auto &it : graphene::tools::log::logBuffer) {
                    int severity = 0;
                    ImU32 textColorTemp = ImGui::GetColorU32(ImGuiCol_Text);
                    switch (it.severity) {
                        case graphene::tools::log::LogEntry::Severity::ERR:
                            ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(168, 48, 44, 255));
                            severity = 4;
                            break;
                        case graphene::tools::log::LogEntry::Severity::WARNING:
                            ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(168, 129, 44, 255));
                            severity = 3;
                            break;
                        case graphene::tools::log::LogEntry::Severity::INFO:
                            ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(102, 168, 44, 255));
                            severity = 2;
                            break;
                        case graphene::tools::log::LogEntry::Severity::DEBUG:
                            ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(44, 151, 168, 255));
                            severity = 1;
                            break;
                        case graphene::tools::log::LogEntry::Severity::VERBOSE:
                            ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(148, 148, 148, 255));
                            severity = 0;
                            break;
                        default:
