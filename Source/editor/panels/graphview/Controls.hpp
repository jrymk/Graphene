...
#include "Common.hpp"
#include "../../../core/GraphIter.hpp"
#include "../../../../Dependencies/imgui/imgui.h"

namespace graphene::editor::graphview::controls {
...
void updateMouse() {
	...
	if (ImGui::IsMouseReleased(ImGuiMouseButton_Left)) {
		...
		if (((leftMouseDownPos.x - ImGui::GetMousePos().x) * (leftMouseDownPos.x - ImGui::GetMousePos().x)) +
					((leftMouseDownPos.y - ImGui::GetMousePos().y) * (leftMouseDownPos.y - ImGui::GetMousePos().y)) <=
				25.0 &&
	...
	if (ImGui::IsMouseReleased(ImGuiMouseButton_Right)) {
		...
		if (((rightMouseDownPos.x - ImGui::GetMousePos().x) * (rightMouseDownPos.x - ImGui::GetMousePos().x)) +
				((rightMouseDownPos.y - ImGui::GetMousePos().y) * (rightMouseDownPos.y - ImGui::GetMousePos().y)) <=
			25.0) {
