...
void drawGrid() {
	drawGridLines(true, View::mapToCanvas(0.0, 0.0).x, View::mapToCanvas(pow(10, -(int)ceil(log10(View::zoomLevel)))),
				  -View::canvasSize.x / 2.0f, View::canvasSize.x / 2.0f, -View::canvasSize.y / 2.0f,
				  View::canvasSize.y / 2.0f, IM_COL32(90, 90, 90, 80), 2.0);
	drawGridLines(true, View::mapToCanvas(0.0, 0.0).x,
				  View::mapToCanvas(pow(10, -(int)ceil(log10(View::zoomLevel)) - 1)), -View::canvasSize.x / 2.0f,
				  View::canvasSize.x / 2.0f, -View::canvasSize.y / 2.0f, View::canvasSize.y / 2.0f,
				  IM_COL32(90, 90, 90, 80), 1.0);
	drawGridLines(false, View::mapToCanvas(0.0, 0.0).y, View::mapToCanvas(pow(10, -(int)ceil(log10(View::zoomLevel)))),
				  -View::canvasSize.y / 2.0f, View::canvasSize.y / 2.0f, -View::canvasSize.x / 2.0f,
				  View::canvasSize.x / 2.0f, IM_COL32(90, 90, 90, 80), 2.0);
	drawGridLines(false, View::mapToCanvas(0.0, 0.0).y,
				  View::mapToCanvas(pow(10, -(int)ceil(log10(View::zoomLevel)) - 1)), -View::canvasSize.y / 2.0f,
				  View::canvasSize.y / 2.0f, -View::canvasSize.x / 2.0f, View::canvasSize.x / 2.0f,
				  IM_COL32(90, 90, 90, 80), 1.0);
}
...
void drawEdges() {
	// edge drawing
	::graphene::core::EdgeIter it(Graphene::core->getGraphObj());
	while (it.next()) {
		ImGui::GetWindowDrawList()->AddLine(View::mapToCanvas(it.u->getCoord()), View::mapToCanvas(it.v->getCoord()),
	...
	if (Controls::leftMouseDownVertex != nullptr) {
		if (Controls::hoveredVertex != nullptr) {
			if (Graphene::core->getGraphObj()->isAdjacent(Controls::leftMouseDownVertex, Controls::hoveredVertex) ||
				...
				ImGui::GetWindowDrawList()->AddLine(View::mapToCanvas(Controls::leftMouseDownVertex->getCoord()),
			...
			ImGui::GetWindowDrawList()->AddLine(View::mapToCanvas(Controls::leftMouseDownVertex->getCoord()),
												ImGui::GetIO().MousePos, IM_COL32(0, 255, 0, 60),
...
void drawVertices() {
	for (auto& component : Graphene::core->getGraphObj()->components) {
		::graphene::core::ComponentVertexIter it(component);
		while (it.next()) {
			ImVec2 vertexScreenCoord(float(View::canvasOrigin.x - (View::centerContext.x - it.v->getCoord().x) *
																	  View::canvasFrameSize * View::zoomLevel),
									 float(View::canvasOrigin.y + (View::centerContext.y - it.v->getCoord().y) *
																	  View::canvasFrameSize * View::zoomLevel));
			...
			::Gui::AlignedText(View::mapToCanvas(it.v->getCoord()), {0.5f, 0.5f}, {200.0f, 100.0f}, it.v->name,
							   ImGui::GetDefaultFont(),
							   float((36.0 / 54.0) * pow(View::zoomLevel, 0.1) *
									 ((it.v == Controls::rightMouseDownVertex) ? 1.1 : 1.0)),
							   IM_COL32(15, 15, 15, 255));

			if (it.v == Graphene::core->getGraphObj()->debugVertexHighlight) {
				ImGui::GetWindowDrawList()->AddCircleFilled(
					vertexScreenCoord, float(20.0 * pow(View::zoomLevel, 0.1) + 12.0),
					ImGui::ColorConvertFloat4ToU32(ImVec4(1.0f, 1.0f, 1.0f, 0.4f)));
			}

			if (it.v == Controls::leftMouseSelectionVertex) {
				ImGui::GetWindowDrawList()->AddCircle(vertexScreenCoord, float(20.0 * pow(View::zoomLevel, 0.1) + 12.0),
													  ImGui::ColorConvertFloat4ToU32(ImVec4(1.0f, 1.0f, 1.0f, 1.0f)), 0,
													  2.0);
			}
		}
	}
	...
	if (Controls::hoveredVertex != nullptr) {
		ImGui::GetWindowDrawList()->AddCircle(
			ImVec2(float(View::canvasOrigin.x - (View::centerContext.x - Controls::hoveredVertex->getCoord().x) *
													View::canvasFrameSize * View::zoomLevel),
				   float(View::canvasOrigin.y + (View::centerContext.y - Controls::hoveredVertex->getCoord().y) *
													View::canvasFrameSize * View::zoomLevel)),
			float(25.0 * pow(View::zoomLevel, 0.1)), IM_COL32(150, 150, 255, 100), 0,
			float(5.0 * pow(View::zoomLevel, 0.1)));
	}
}
