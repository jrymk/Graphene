...
namespace imgui_addons {
    ...
    bool ImGuiFileBrowser::showFileDialog(const std::string& label, const DialogMode mode, const ImVec2& sz_xy,
        ...
        ImGui::SetNextWindowSize(ImVec2(std::max(sz_xy.x, min_size.x), std::max(sz_xy.y, min_size.y)),
                                 ImGuiCond_Appearing);
    ...
    bool ImGuiFileBrowser::renderFileListRegion() {
        ...
        for (std::vector<const Info*>::size_type i = 0; i < filtered_dirs.size(); i++) {
            if (!filtered_dirs[i]->is_hidden || show_hidden) {
                items++;
                if (ImGui::Selectable(filtered_dirs[i]->name.c_str(), selected_idx == static_cast<int>(i) && is_dir,
        ...
        for (std::vector<const Info*>::size_type i = 0; i < filtered_files.size(); i++) {
            if (!filtered_files[i]->is_hidden || show_hidden) {
                items++;
                if (ImGui::Selectable(filtered_files[i]->name.c_str(), selected_idx == static_cast<int>(i) && !is_dir,
                                      ImGuiSelectableFlags_AllowDoubleClick)) {
    ...
    bool ImGuiFileBrowser::renderInputTextAndExtRegion() {
        ...
        if (ImGui::InputTextWithHint("##FileNameInput", "Type a name...", &input_fn[0], 256,
        ...
        if (show_inputbar_combobox && (ImGui::GetFocusedFocusScope() == focus_scope_id ||
            ...
            if (ImGui::ListBoxHeader("##InputBarComboBoxList", listbox_size)) {
                ...
                for (auto& element : inputcb_filter_files) {
                    if (ImGui::Selectable(element.get().c_str(), false, ImGuiSelectableFlags_NoHoldingActiveID |
    ...
    void ImGuiFileBrowser::renderExtBox() {
        ...
        if (ImGui::BeginCombo("##FileTypes", selected_label)) {
            ...
            for (std::vector<std::string>::size_type i = 0; i < valid_exts.size(); i++) {
                if (ImGui::Selectable(valid_exts[i].c_str(),
                                      selected_ext_idx == static_cast<int>(i) && !show_all_files &&
                                      !show_files_with_valid_extensions)) {
            ...
            for (std::vector<Info>::size_type i = 0; i < subfiles.size(); ++i) {
                if (show_files_with_valid_extensions) {
                    if (filter.PassFilter(subfiles[i].name.c_str())) {
                        std::string ext =
                                subfiles[i].name.find_last_of('.') == std::string::npos ? "" : subfiles[i].name.substr(
                                        subfiles[i].name.find_last_of('.'));
    ...
    bool ImGuiFileBrowser::showReplaceFileModal() {
        ...
        if (ImGui::BeginPopupModal(repfile_modal_id.c_str(), nullptr,
            ...
            ImGui::SetCursorPosX(ImGui::GetCursorPosX() + ImGui::GetWindowWidth() / 2.0f - buttons_width / 2.0f -
