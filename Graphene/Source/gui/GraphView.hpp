...
namespace Gui {
    namespace GraphView {
        ...
        void canvasBegin() {
            ...
            canvasDisplaySize = std::max(
                    std::min(drawSize.x - canvasMargin.x * 2.0f, drawSize.y - canvasMargin.y * 2.0f), FLT_MIN);
            ...
            ImGui::InvisibleButton("canvas", drawSize,
        ...
        void drawOverlay(Graphene::Core* core) {
            ...
            ImGui::GetWindowDrawList()->AddText(
                    ...
                    "Drag from vertex to vertex with left mouse button to create/delete edge, drag vertex with right mouse button to nudge vertex position",
                    0);
        ...
        void updateHoveredVertex(Graphene::Core* core, Graphene::Graph* graph) {
            ...
            if (ImGui::IsMouseReleased(ImGuiMouseButton_Left)) {
                if (leftMouseDownVertex != nullptr && hoveredVertex != nullptr) {
                    if (graph->isAdjacent(leftMouseDownVertex, hoveredVertex))
                        graph->deleteEdge(leftMouseDownVertex, hoveredVertex);
                    else if (graph->isAdjacent(hoveredVertex, leftMouseDownVertex))
                        graph->deleteEdge(hoveredVertex, leftMouseDownVertex);
                    else
                        graph->newEdge(leftMouseDownVertex, hoveredVertex);
        ...
        void drawGrid() {
...

            {
                ...
                const float startingSnapGridCoord =
                        centerSnapGridCoord - (int) (drawSize.x / screenGridSpacing / 2.0f + 2.0f) * screenGridSpacing;
                ...
                for (float x = startingSnapGridCoord; x <= endingSnapGridCoord; x += screenGridSpacing) {
                    drawList->AddLine(ImVec2(x, centerDrawCoord.y - drawSize.y / 2.0f),
                                      ImVec2(x, centerDrawCoord.y + drawSize.y / 2.0f),
            ...
            }
            {
                ...
                const float startingSnapGridCoord =
                        centerSnapGridCoord - (int) (drawSize.y / screenGridSpacing / 2.0f + 2.0f) * screenGridSpacing;
                const float endingSnapGridCoord =
                        centerSnapGridCoord + (int) (drawSize.y / screenGridSpacing / 2.0f + 2.0f) * screenGridSpacing;

                int lineDrawLimit = 500;
                for (float y = startingSnapGridCoord; y <= endingSnapGridCoord; y += screenGridSpacing) {
                    drawList->AddLine(ImVec2(centerDrawCoord.x - drawSize.x / 2.0f, y),
                                      ImVec2(centerDrawCoord.x + drawSize.x / 2.0f, y),
                                      IM_COL32(90, 90, 90, 80), 1.0f);
                    if (!(--lineDrawLimit))
                        break;
                }
        ...
        void drawEdges(Graphene::Graph* graph) {
            ...
            while (it.next()) {
                ImGui::GetWindowDrawList()->AddLine(
                        ImVec2(centerDrawCoord.x -
                               (centerMapped.x - it.u->getCoord().x) * canvasDisplaySize * zoomLevel,
                               centerDrawCoord.y +
                               (centerMapped.y - it.u->getCoord().y) * canvasDisplaySize * zoomLevel),
                        ImVec2(centerDrawCoord.x -
                               (centerMapped.x - it.v->getCoord().x) * canvasDisplaySize * zoomLevel,
                               centerDrawCoord.y +
                               (centerMapped.y - it.v->getCoord().y) * canvasDisplaySize * zoomLevel),
            ...
            if (leftMouseDownVertex != nullptr) {
                if (hoveredVertex != nullptr) {
                    if (graph->isAdjacent(leftMouseDownVertex, hoveredVertex) ||
                        graph->isAdjacent(hoveredVertex, leftMouseDownVertex)) {
        ...
        void drawVertices(Graphene::Graph* graph) {
            for (auto &component : graph->components) {
                ...
                while (it.next()) {
                    ImVec2 vertexScreenCoord(
                            centerDrawCoord.x - (centerMapped.x - it.v->getCoord().x) * canvasDisplaySize * zoomLevel,
                    ...
                    ImGui::GetWindowDrawList()->AddCircleFilled(
                            vertexScreenCoord,
                            20.0f * powf(zoomLevel, 0.1) * ((it.v == rightMouseDownVertex) ? 1.1f : 1.0f),
                    ...
                    ImGui::SetWindowFontScale(
                            (36.0f / 54.0f) * powf(zoomLevel, 0.1) * ((it.v == rightMouseDownVertex) ? 1.1f : 1.0f));
                    ...
                    if (it.v == component->root) {
                        ...
                        ImGui::GetWindowDrawList()->AddCircle(
                                vertexScreenCoord,
                                25.0f * powf(zoomLevel, 0.1),
                                IM_COL32(255, 255, 0, 255), 0, 3.0f * powf(zoomLevel, 0.1));
            ...
            if (hoveredVertex != nullptr) {
                ImGui::GetWindowDrawList()->AddCircle(
                        ImVec2(centerDrawCoord.x -
                               (centerMapped.x - hoveredVertex->getCoord().x) * canvasDisplaySize * zoomLevel,
                               centerDrawCoord.y +
                               (centerMapped.y - hoveredVertex->getCoord().y) * canvasDisplaySize * zoomLevel
                        ),
