...
namespace gfn {
    ...
    void VertexProps::serialize(binn* object) {
        ...
        binn_object_set_uint32(object, fillColor.key.c_str(), fillColor.value);
        binn_object_set_double(object, radius.key.c_str(), radius.value);
        binn_object_set_uint32(object, outlineColor.key.c_str(), outlineColor.value);
    ...
    void VertexProps::deserialize(void* object) {
        ...
        binn_object_foreach(object, key, value) {
            ...
            else if (key == fillColor.key && value.type == BINN_UINT32) fillColor.value = value.vuint32;
            else if (key == radius.key && value.type == BINN_DOUBLE) radius.value = value.vdouble;
            else if (key == outlineColor.key && value.type == BINN_UINT32) outlineColor.value = value.vuint32;
